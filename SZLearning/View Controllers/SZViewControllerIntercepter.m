...
#import "SZViewControllerIntercepter.h"

/* Method Swizzling 手段实现方法拦截 */
#import <Aspects.h>
#import <UIKit/UIKit.h>

@implementation SZViewControllerIntercepter

+ (void)load
{
    /* + (void)load 会在应用启动的时候自动被runtime调用，通过重载这个方法来实现最小的对业务方的“代码入侵” */
    [super load];
    [SZViewControllerIntercepter sharedInstance];
}
...
+ (instancetype)sharedInstance
{
    ...
    static SZViewControllerIntercepter *sharedInstance;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[SZViewControllerIntercepter alloc] init];
    });
